import { LineType, NoPlugin, ResolvedCoreOptions, ChartGLOptions, ChartGLOptionsBase, ChartGLPlugins, ChartGLSeriesOptions } from '../options';
...
type TPluginStates<TPlugins> = { [P in keyof TPlugins]: TPlugins[P] extends ChartGLPlugin<infer TState> ? TState : never };

function completeSeriesOptions(s: Partial<ChartGLSeriesOptions>): ChartGLSeriesOptions {
    s.data = s.data ? DataPointsBuffer._from_array(s.data) : new DataPointsBuffer();
    Object.setPrototypeOf(s, defaultSeriesOptions);
    return s as ChartGLSeriesOptions;
}

function completeOptions(el: Element, options?: ChartGLOptionsBase): ResolvedCoreOptions {
    const dynamicDefaults = {
        series: [] as ChartGLSeriesOptions[],
...
export default class ChartGL<TPlugins extends ChartGLPlugins=NoPlugin> {
